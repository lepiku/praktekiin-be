...
class Pengguna(AbstractUser):
    ...
    peran = models.CharField(
        max_length=16, blank=True, choices=Peran.choices, default=Peran.PASIEN
    )
...
class JadwalKerja(models.Model):
    pengguna = models.ForeignKey(
        Pengguna, related_name="daftar_jadwal_kerja", on_delete=models.CASCADE
    )
...
class Notifikasi(models.Model):
    ...
    daftar_penerima = models.ManyToManyField(Pengguna, related_name="daftar_notifikasi")
